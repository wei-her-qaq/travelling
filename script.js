...
(function(){
  ...
  var CHARS=[
    ...
    {name:'米娜',title:'跨卷旅人',img:'images/characters/mina-01.jpg',detail:'在 Vol 1 魔法师之国登场，Vol 12 再次出现——全系列中跨越多卷反复登场的少数角色之一。\n\n登场：Vol 1,12\nMAL: 189253'},
  ...
  var N=CHARS.length;
  var R_OUT=240;        // big radius
  var R_IN=110;         // small radius
  var SA=360/N;         // sector angle 27.69°
  var VISIBLE=2;        // ±2 = max 5 visible
  var current=0;
  var animating=false;

  // ---- ClipPath: show only left ~83° arc ----
  // viewBox is -280,-190 300,380 → center at (0,0)
  // We show a pie wedge from 140° to 220° (left side, 80°)
  // Build as SVG path inside clipPath
  var clip=document.createElementNS(SVG_NS,'clipPath');
  clip.setAttribute('id','wheelClip');
  var clipPath=document.createElementNS(SVG_NS,'path');
  var ca0=140*Math.PI/180, ca1=220*Math.PI/180;
  var cx0=R_OUT*Math.cos(ca0), cy0=R_OUT*Math.sin(ca0);
  var cx1=R_OUT*Math.cos(ca1), cy1=R_OUT*Math.sin(ca1);
  var ix0=R_IN*Math.cos(ca0), iy0=R_IN*Math.sin(ca0);
  var ix1=R_IN*Math.cos(ca1), iy1=R_IN*Math.sin(ca1);
  clipPath.setAttribute('d',
    'M '+ix0+' '+iy0+
    ' L '+cx0+' '+cy0+
    ' A '+R_OUT+' '+R_OUT+' 0 0 1 '+cx1+' '+cy1+
    ' L '+ix1+' '+iy1+
    ' A '+R_IN+' '+R_IN+' 0 0 0 '+ix0+' '+iy0+' Z'
  );
  clip.appendChild(clipPath);
  defs.appendChild(clip);

  // ---- Patterns for image fills ----
  CHARS.forEach(function(ch,i){
    var pat=document.createElementNS(SVG_NS,'pattern');
    pat.setAttribute('id','p'+i);
  ...
  // ---- Build sectors ----
  function ringPath(i){
    var a0=(i*SA-SA/2)*Math.PI/180;
    var a1=(i*SA+SA/2)*Math.PI/180;
    var x0i=R_IN*Math.cos(a0),y0i=R_IN*Math.sin(a0);
    var x0o=R_OUT*Math.cos(a0),y0o=R_OUT*Math.sin(a0);
    var x1o=R_OUT*Math.cos(a1),y1o=R_OUT*Math.sin(a1);
    var x1i=R_IN*Math.cos(a1),y1i=R_IN*Math.sin(a1);
    return 'M '+x0i+' '+y0i+' L '+x0o+' '+y0o+
           ' A '+R_OUT+' '+R_OUT+' 0 0 1 '+x1o+' '+y1o+
           ' L '+x1i+' '+y1i+
           ' A '+R_IN+' '+R_IN+' 0 0 0 '+x0i+' '+y0i+' Z';
  }
  function innerPath(i){
    var a0=(i*SA-SA/2)*Math.PI/180;
    var a1=(i*SA+SA/2)*Math.PI/180;
    var x0=R_IN*Math.cos(a0),y0=R_IN*Math.sin(a0);
    var x1=R_IN*Math.cos(a1),y1=R_IN*Math.sin(a1);
    return 'M 0 0 L '+x0+' '+y0+' A '+R_IN+' '+R_IN+' 0 0 1 '+x1+' '+y1+' Z';
  }
  ...
  CHARS.forEach(function(ch,i){
    ...
    ring.setAttribute('class','wheel-sector-ring');
    ring.setAttribute('d',ringPath(i));
    ring.setAttribute('fill','url(#p'+i+')');
    g.appendChild(ring);

    var inner=document.createElementNS(SVG_NS,'path');
    inner.setAttribute('class','wheel-sector-inner');
    inner.setAttribute('d',innerPath(i));
    g.appendChild(inner);

    // Name label at mid-angle, inner radius
    var midA=(i*SA)*Math.PI/180;
    var midR=R_IN*0.65;
    var label=document.createElementNS(SVG_NS,'text');
    label.setAttribute('class','wheel-sector-label');
    label.setAttribute('x',midR*Math.cos(midA));
    label.setAttribute('y',midR*Math.sin(midA)-5);
    label.textContent=ch.name;
    g.appendChild(label);

    // Title label below name
    var title=document.createElementNS(SVG_NS,'text');
    title.setAttribute('class','wheel-sector-title');
    title.setAttribute('x',midR*Math.cos(midA));
    title.setAttribute('y',midR*Math.sin(midA)+5);
  ...
  // ---- Update visibility & rotation ----
  function update(){
    // Rotate so current sector is at 180° (left center)
    var targetRot=180-(current*SA);
    ...
    for(var i=0;i<N;i++){
      ...
      if(abs<=VISIBLE){
        ...
        g.style.opacity=abs===0?1:abs===1?0.6:abs===2?0.3:0.15;
      }else{
  ...
  function snapTo(idx){
    ...
    function anim(ts){
      ...
      var rot=180-(current*SA);
      rotateG.setAttribute('transform','rotate('+rot+')');
      // Live visibility update
      var ci=Math.round(current)%N;
      if(ci<0)ci+=N;
      for(var i=0;i<N;i++){
        var rel=i-ci;
        if(rel>N/2)rel-=N;
  ...
  function rotateBy(dir){snapTo(current+dir)}
  ...
  var touchX=null;
  svg.addEventListener('touchstart',function(e){touchX=e.touches[0].clientX},{passive:true});
  ...
  svg.addEventListener('touchend',function(){touchX=null},{passive:true});
